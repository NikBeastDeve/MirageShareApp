...
extension MirageClientService {
    ...
    func handleDisconnect(reason: String, state: ConnectionState, notifyDelegate: Bool) async {
        ...
        // Clean up any pending stream start continuation to prevent leaks.
        if let continuation = streamStartedContinuation {
            continuation.resume(throwing: MirageError.protocolError("Disconnected: \(reason)"))
