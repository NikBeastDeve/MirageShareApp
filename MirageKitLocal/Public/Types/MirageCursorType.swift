    ...
    init?(from cursor: NSCursor?) {
        ...
        if cursorData == NSCursor.arrow.image.tiffRepresentation { self = .arrow } else if cursorData == NSCursor.iBeam.image.tiffRepresentation {
            ...
            self = .contextualMenu
                  ...
                  cursorData == NSCursor.frameResize(position: .topRight, directions: .inward).image.tiffRepresentation
            || cursorData == NSCursor.frameResize(position: .topRight, directions: .outward).image
            .tiffRepresentation {
            self = .resizeNorthEast
        } else if #available(macOS 15.0, *),
                  cursorData == NSCursor.frameResize(position: .topLeft, directions: .inward).image.tiffRepresentation
            || cursorData == NSCursor.frameResize(position: .topLeft, directions: .outward).image
            .tiffRepresentation {
            self = .resizeNorthWest
        } else if #available(macOS 15.0, *),
                  cursorData == NSCursor.frameResize(position: .bottomRight, directions: .inward).image
            .tiffRepresentation
            || cursorData == NSCursor.frameResize(position: .bottomRight, directions: .outward).image
            .tiffRepresentation {
            self = .resizeSouthEast
        } else if #available(macOS 15.0, *),
                  cursorData == NSCursor.frameResize(position: .bottomLeft, directions: .inward).image
            .tiffRepresentation
            || cursorData == NSCursor.frameResize(position: .bottomLeft, directions: .outward).image
            .tiffRepresentation {
            self = .resizeSouthWest
        } else if #available(macOS 15.0, *),
                  cursorData == NSCursor.frameResize(position: .topRight, directions: .all).image.tiffRepresentation
            || cursorData == NSCursor.frameResize(position: .bottomLeft, directions: .all).image
            .tiffRepresentation {
            self = .resizeNESW
        } else if #available(macOS 15.0, *),
                  cursorData == NSCursor.frameResize(position: .topLeft, directions: .all).image.tiffRepresentation
