...
extension HEVCDecoder {
    ...
    // Nonisolated(unsafe) because we're working with raw pointers in a constrained scope
    // This function is called from an actor-isolated context
    private func updateFormatDescription(vpsData: Data, spsData: Data, ppsData: Data) throws {
        // Use contiguous storage to avoid nested closures
        let vpsBytes = vpsData.withUnsafeBytes { Array($0.bindMemory(to: UInt8.self)) }
        let spsBytes = spsData.withUnsafeBytes { Array($0.bindMemory(to: UInt8.self)) }
        let ppsBytes = ppsData.withUnsafeBytes { Array($0.bindMemory(to: UInt8.self)) }
